...
public abstract class Singleton<T> : MonoBehaviour where T : Singleton<T>
{
    ...
    protected virtual void Awake()
    {
        if (_instance != null)
        {
            Debug.LogWarning("Only 1 " + typeof(T) + " can exist in the scene.  Self-destructing.");
            Destroy(this.gameObject);
        }
        //  Here we cast the instance type to the Generic type T which will be set by inheriters
        _instance = this as T;
        //  Since we are using the Awake() function for our own purposes,
        //  we allow the inheriter to instead call Init() to perform the same functions
        Init();
    }
    ...
    //  Inheriters can implement this function to replace Awake()
